...
std::vector<std::string> getAllEvmBytecodeFiles() {
  ...
  const char *DirEnv = std::getenv("EVM_BYTECODE_DIR");
  std::string DirPath = DirEnv && std::strlen(DirEnv) > 0 ? DirEnv : "../tests/evm_bytecode";

  if (!std::filesystem::exists(DirPath)) {
    std::cerr << "EVM bytecode directory does not exist: " << DirPath << std::endl;
    return Files;
  }
...
std::string readAnswerFile(const std::string &FilePath) {
  ...
  std::string DirPath = (SepPos == std::string::npos) ? std::string() : FilePath.substr(0, SepPos + 1);

  std::string FileName = (SepPos == std::string::npos) ? FilePath : FilePath.substr(SepPos + 1);
...
TEST_P(EVMSampleTest, ExecuteSample) {
  ...
  for (size_t I = 0; I < Hex.size(); I += 2) {
    uint8_t ByteVal = static_cast<uint8_t>(std::stoul(Hex.substr(I, 2), nullptr, 16));
  ...
  EXPECT_EQ(Ctx.getCurFrame(), nullptr) << "Frame should be deallocated after execution";
}
