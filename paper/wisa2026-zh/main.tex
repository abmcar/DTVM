...
语义中间表示 dMIR (Deterministic Middle IR)，其 JIT 中窥孔重写规则
若未经等价性验证即落盘，则可能引发链分叉。然而 LLVM IR 等通用中间表示以多返回值
结构间接承载位级副产物，使一类规则难以在 Alive~\cite{alive} 等自动验证
框架内以单条直接表述；共识关键场景下工程侧只能放弃、未验部署或人工证明，
分别损失性能、危及共识或难规模化。瓶颈在中间表示表达力而非求解器容量，
本文通过 IR 侧一等位向量算子扩展来扩大可自动验证窥孔覆盖面：不改求解器、
不放宽差分门槛，将相关模式建为 dMIR 一等算子，使规则进入 Z3 等价性判据。
构建两级窥孔重写系统：dMIR 层 \textbf{70} 条经 Z3 逐条校验、x86 层
\textbf{13} 条由对照测试覆盖；配套 \textbf{853} 条候选的独立合成管线刻画
准入容量（代数枚举 \textbf{98.7\%}、进位模板 \textbf{21.8\%} 准入率）。
相对未启用本文规则的 DTVM 基线，evmone-bench \textbf{27} 基准加速比几何
均值 \textbf{+7.24\%}（95\% 置信区间 [+2.59\%, +12.11\%]），峰值
\textbf{+25.79\%}；\textbf{5884} 条差分测试在启用/关闭两侧状态与 gas
逐字节一致。
\vspace{0.3em}
