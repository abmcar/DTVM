...
语义中间表示 dMIR (deterministic Middle IR)，其 JIT 中窥孔规则一旦等价性
出错即可引发链分叉。然而 LLVM IR 等通用中间表示以多返回值结构间接承载位级
副产物，使一类规则难以在 Alive 等自动验证框架内以单条直接表述；共识关键
场景下工程侧只能放弃、未验部署或人工证明，分别损失性能、危及共识或难规模
化。瓶颈在中间表示表达力而非求解器容量，本文通过 IR 侧一等位向量算子扩展
来扩大可自动验证窥孔覆盖面：不改求解器、不放宽差分门槛，将相关模式建为
dMIR 一等算子，使规则进入 Z3 等价性判据。构建两级窥孔重写系统：dMIR 层
\textbf{70} 条经 Z3 逐条校验、x86 层 \textbf{13} 条由对照测试覆盖；配套
\textbf{853} 条候选的独立合成管线刻画准入容量（98.7\%/21.8\% 准入率）。
evmone-bench \textbf{27} 基准加速比几何均值 \textbf{+7.24\%}（95\% 置信
区间 [+2.59\%, +12.11\%]），峰值 \textbf{+25.79\%}；\textbf{5884} 条差分
测试在启用/关闭两侧状态与 gas 口径一致。
\vspace{0.3em}
