...
链分叉。LLVM IR 等通用中间表示以多返回值结构间接表达进位位，自动等价验证工具对
进位链窥孔规则的覆盖因此受限。本文在 DTVM 的 dMIR 中将带进位加法 \texttt{ADC} 与带借位减法
\texttt{SBB} 建模为一等位向量算子，通过候选合成与 Z3 位向量等价性准入输出
\textbf{70} 条 dMIR 规则；另有 \textbf{13} 条 x86 代码生成层规则由对照
测试套件覆盖。在 evmone-bench 的 \textbf{27} 个 EVM 基准上，加速比几何均值
为 \textbf{+7.24\%}（95\% 置信区间 [+2.59\%, +12.11\%]），峰值加速
\textbf{+25.79\%}。启用与关闭规则集两侧 \textbf{5884} 条对照测试在状态与
gas 口径上逐项一致。与 Albert 等人 CAV 2023 字节码块级规则集在通用位向量
恒等式上仅 \textbf{8.4\%} 交集，其余作用于其无法触及的 JIT 中间表示与
机器码层。
\vspace{0.3em}
