...
\title{\bfseries 面向区块链信息系统执行层的可验证窥孔优化：\\dMIR 进位链算子与 SMT 等价性校验}
\author{匿名作者~~匿名单位}
\date{}
...
难以作为单条 peephole 规则直接表达（见表~\ref{tab:cannot-express}）。本文在 DTVM 的 dMIR 中将带进位加法
\texttt{ADC} 与带借位减法 \texttt{SBB} 建模为 dMIR 原生算子，构建两级
...
逐项一致。与 Albert 等人 CAV 2023 字节码块级规则集在通用位向量恒等式上
仅 \textbf{8.4\%} 交集，其余作用于其无法触及的 JIT 中间表示与机器码层。
\vspace{0.3em}
