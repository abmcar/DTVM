...
现有通用 IR 以间接方式表达进位位，限制了自动等价验证的适用范围。本文在 DTVM 的
dMIR 中把带进位加减法 \texttt{adc}/\texttt{sbb} 设计为一等位向量算子，构建了
\textbf{70} 条 dMIR 层（Z3 位向量等价性校验）与 \textbf{13} 条 x86 层（对照测试
套件覆盖）的两级窥孔重写系统。在 27 个标准 EVM 基准上，加速比的几何均值为
\textbf{+7.24\%}（95\% 置信区间 [+2.59\%, +12.11\%]，下界显著大于零），峰值加速
\textbf{+25.79\%}；同口径下观察到 4 个 \texttt{memory\_grow\_mstore} 微基准回归
\textbf{7.74\%}--\textbf{8.85\%}，其余回归均在 3\% 以内。\textbf{5884} 条对照
测试在启用/关闭规则集两侧全部通过且结果一致。与 Albert 等人 CAV 2023 字节码块级
验证规则集相比，重合度仅 \textbf{8.4\%}，两套规则指向不同抽象层次。
\vspace{0.3em}
