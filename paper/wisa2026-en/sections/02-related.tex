...
chains. Search-based superoptimization baselines include
STOKE~\cite{stoke} (stochastic) and Stratified
Synthesis~\cite{stratified} (enumerative); SMT-based peephole checking is
...
simplifications (including the ADC/SBB carry-chain rewrites enabled by
the first-class carry operators) plus 13 x86 cleanup rewrites; the
carry-chain and x86 components fall outside the bytecode-block
abstraction, while some Boolean identities overlap in scope but are
absent from the verified \texttt{forves} set.
