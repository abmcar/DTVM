...
fixed-width modular bit-vector semantics. Its enumerator also reaches
all production left-hand sides (\textbf{70/70}, after canonicalization
tuning informed by an initial 0/70 baseline; Sec.~\ref{sec:coverage-scope}),
including carry-chain forms that resist direct formulation in LLVM IR.
...
differential testing; across 27 evmone-bench~\cite{evmone} workloads,
the 83-rule system attains a \textbf{+7.24\%} geometric-mean speedup
and passes \textbf{5{,}884} paired differential tests (Cancun statetests
plus EVM-spec unit tests) with byte-identical final state and matching
gas accounting within suite coverage.
\end{itemize}
